...
// Type: QianChang.ZhanShi.能力技.战士能力技动乱
...
{
    public class 战士能力技动乱 : ISlotResolver
    {
        public int Check()
        {
            ...
            if (战士ACR入口.QT.GetQt("优先动乱") && SpellExtension.IsReadyWithCanCast(SpellHelper.GetSpell(7387U)))
                return 1;
            return SpellExtension.IsReadyWithCanCast(SpellHelper.GetSpell(7387U)) && 战士SpellHelper.自身存在Buff(2677U) ? 0 : -2;
        }

        public void Build(Slot slot) => slot.Add(SpellHelper.GetSpell(7387U));
    }
}